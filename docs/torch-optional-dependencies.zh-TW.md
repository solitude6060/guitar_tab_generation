...
- `torch`、`torchaudio` 與 `torchcodec`：共用 PyTorch runtime / audio loading packages。
...
CPU-safe real route smoke。這會在 model cache 建立很短的本機 smoke artifact，並寫出 `f0_calibration.json`：

```bash
uv run guitar-tab-generation torch-smoke --route torchcrepe-f0 --run --device cpu --json
```

共享 RTX 4090 主機上的 torchcrepe GPU-sensitive 檢查必須明確 opt-in 並受 VRAM gate 保護：
...
Demucs 仍是後續 route，應使用較高 VRAM guard：
